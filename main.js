...
function validate_windows_section(y) {
  ...
  ws.forEach((w, key) => {
    ...
    if (!w.viewports || w.viewports.length == 0) {
      console.log('window definition [', key, '] lacks viewport list.');
      windows_ok = false;
    }
    ...
    if (missings.length > 0) {
      console.log('window definition [', key, '] refers to viewport(s) which are missing from the viewports collection: ', missings);
      windows_ok = false;
    }
  });
  return windows_ok;
...
function validate_space_section(y) {
  ...
  ms.forEach(m => {
    ...
    if (missings.length > 0) {
      console.log('Machine definition [', m.name, '] refers to window(s) which are missing from the windows collection: ', missings);
      space_ok = false;
    }
  });
  return space_ok;
...
function convert_roomfeldscreen_to_space(room, feld, screen) {
  ...
  if (is_kombifeld_protein(feld)) {
    ...
    space.machines[this_machine_index].windows = window_names_from_feld_protein(feld);
  }
